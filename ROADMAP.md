...
- ✅ **Flat-by-default table that pivots on demand**: with **no Nested Layers** (default) it shows the full flat view — one column per distinct value of every category, all visible immediately. Adding **Nested Layers** (ordered dropdowns — layer 1 outermost, `+ Add layer` appends 2, 3…) *pivots* the same data into nested value-combinations with a spanning header (`GroupedHeaderView`, e.g. condition over replicate). Fixed `feature` + `unique` columns lead. Each cell = the feature's quantity **averaged across every file in that column** (zeros/missing never averaged in); ≥2-file averages shaded **grey**. Sized to content, no stretch. Independent of the panel-1 organizer **and of the replicate designation**; layers persist.
