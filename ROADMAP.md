...
- ✅ **Flat value table**: fixed `feature` + `unique` columns, then **one column per distinct value of every experimental-setup category** (condition values, fraction values, replicate values… side by side, no nesting). Each cell = the feature's quantity **averaged across all files matching that value** (zeros/missing never averaged in); a cell averaged from ≥2 files is shaded **grey**. Column chips **reorder** (◀ ▶) and **toggle** categories — unchecking a category just drops its value-columns (never an 'All' column). Columns sized to content (re-tighten on toggle), no stretch. Independent of the panel-1 organizer; arrangement persists.
