...
- ✅ **Cross-tab pivot table**: with **no Nested Layers** it's flat — one column per distinct value of every category (A, B, 1, 2, 3…). Adding a layer promotes that category to a nested **super-column** and organizes every *other* category's value-columns under it (`GroupedHeaderView` spanning header): layer=LETTERS turns A,B,1,2,3 into A:(1,2,3), B:(1,2,3); more layers nest further. A pending "(choose column)" dropdown never auto-pivots — a column is committed only via `+ Add layer`. Fixed `feature` + `unique` columns lead. Each cell = the feature's quantity **averaged across the files matching that super-path and sub-value** (zeros/missing never averaged in); ≥2-file averages shaded **grey**. Always opens flat (layers not persisted). Independent of the panel-1 organizer and the replicate designation.
- ✅ **Remove-column control** (right-aligned): a "Remove column" dropdown drops a whole category from the table — its value-columns disappear regardless of nesting, and its layer vanishes with it. Each removed category becomes a `<column> ✕` button that re-introduces it. Table-only (doesn't affect the panel-1 organizer or fold-change); not persisted.
- ✅ **Panel-1 organization caption**: the black text above the organizer describes the categories currently organizing the facet plot (split + x-axis), each value with its sample size (e.g. `condition: keloid (n=14), … · fraction: NaCl (n=17), …`), adapting as the organizer/feature change.
