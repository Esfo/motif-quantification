...
- ✅ **Pivot table**: fixed `feature` + `unique` columns, then **the sample groups themselves as columns** — a nested, spanning header (`GroupedHeaderView`) built from the category *values* (e.g. condition spanning over replicate), quantities as the cells. Column nesting is **entirely user-arranged** via the table's own column chips (every category included by default; reorder with ◀ ▶ = outer↔inner, toggle any off) — independent of the panel-1 organizer. A category toggled off collapses, so files landing in the same leaf are **averaged (non-zero values only)** and the cell is shaded **grey**. Arrangement persists.
