...
- **3D view**: m/z, time, intensity.
  ...
  - **3D (reworked):** ✅ **surface removed entirely** — just the **individual datapoints** (centroid and profile); the 0-intensity floor is gone, baseline clean. ✅ Axes mapped **m/z → horizontal (GL-x), time → GL-y, intensity → height**; **camera azimuth −90 puts m/z left→right aligned with panel 2's x**, elevation 30 perspective; footprint **aspect-scaled to fill the rectangular pane**. ✅ **Normalised to panel 2's VISIBLE window** (not the padded load region) and **re-rendered as panel 2 is moved/zoomed**, so its mass/time extent matches panel 2 instead of being "way off". ✅ **Spawn and "align 3D" use the SAME camera** (consistent perspective, no surprise bird's-eye). ✅ **m/z + time side labels are visible again** (dark pill so they read on either theme — plain white was invisible on the light chrome) and screen-fixed so they don't float/drift. ✅ **Datapoints coloured by the panel-2 distribution colour, blended to white at the peak tips**, log/linear via the colour toggle. 🟡 exact pixel-alignment of the m/z extent with panel 2 is still approximate (aspect-based). 🟡 lag reduced (5k cap, surface gone).
