...
- 🟡 When an **MS1 distribution** is clicked, Panel 3 shows the plot from `examples/panel-3-plot.py`: the **charge-state-determination grid** that **links multiple distributions (charge states of one analyte) into one plot of many comparisons** — columns = charge states, **8 rows**: retention time / peak area / charge distances / cross-charge / intensity sum % / adjacency / ppm-to-mean / ppm-error.
  ...
  - ✅ Peak area / cross-charge / intensity-sum% are now **linear bars glued to a 0 baseline** (like the MS2 spectrum), not log — they were misbehaving (bars "hanging from the top", y-scroll zooming the wrong way) because pyqtgraph's BarGraphItem doesn't render correctly in log mode. Their y-axis now starts at 0 at the bottom.
  - ✅ Y axes reliably show their **3 ticks** now that the rows are linear (the log mode was dropping/duplicating ticks).
