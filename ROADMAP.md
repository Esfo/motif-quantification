...
- ✅ **Pivot table**: rows = features; **columns are the sample groups themselves** — a nested, spanning header (`GroupedHeaderView`) built from the category *values* (e.g. condition spanning over replicate), quantities as the cells. Column nesting = the organizer's column/row-split categories (outer→inner) + the designated replicate column innermost; a category left out collapses, so files landing in the same leaf are **averaged (non-zero values only)** and the cell is shaded **grey** to mark the average. `unique`-only still filters peptides.
