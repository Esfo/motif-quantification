...
- **3D view**: m/z, time, intensity.
  ...
  - **3D (reworked):** ✅ **surface removed entirely** — the 3D view is now just the **individual datapoints** for both centroid and profile (the user preferred the centroid look; the 0-intensity floor is gone, baseline clean at the bottom). ✅ Axes mapped **m/z → horizontal (GL-x), time → GL-y, intensity → height**, matching panel 2's orientation; footprint is **aspect-scaled to fill the rectangular pane** instead of a forced square (perfect pixel-alignment of the m/z extent with panel 2 is approximate). ✅ **Initial load and "align 3D" are now the SAME** correct top-down panel-2-aligned view (elevation **89** — not 90, a pyqtgraph singularity that lagged orbit "out of existence"); orbit from there, align snaps back. ✅ **no axes/gnomon** — m/z + time names are screen-fixed white QLabels on the bottom/left sides. ✅ **Datapoints coloured by the panel-2 distribution colour, blended to white at the peak tips** (distribution colour = gradient low end, white = high end), log/linear via the colour toggle. 🟡 lag reduced (5k-point cap, surface gone).
...
- 🟡 When an **MS1 distribution** is clicked, Panel 3 shows the plot from `examples/panel-3-plot.py`: the **charge-state-determination grid** that **links multiple distributions (charge states of one analyte) into one plot of many comparisons** — columns = charge states, **8 rows**: retention time / peak area / charge distances / cross-charge / intensity sum % / adjacency / ppm-to-mean / ppm-error.
  ...
  - ✅ **Every plot in a charge's column now uses that charge's own colour at full opacity** (matching its row-0 mass-vs-time plot) — the cross-charge and ppm-error rows no longer recolour bars by the *other* charge, which (with opacity) had made the colours indistinguishable.
