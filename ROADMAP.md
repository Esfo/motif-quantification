...
- ✅ **Pivot table via Nested Layers**: fixed `feature` + `unique` columns, then a spanning header (`GroupedHeaderView`) of the sample groups. Columns are driven by **Nested Layers** — ordered dropdowns (layer 1 = outermost, `+ Add layer` appends layer 2, 3…), each picking a category; the table pivots by those layers' *values* (e.g. condition over replicate). Each cell = the feature's quantity **averaged across every file in that leaf** (zeros/missing never averaged in); a cell averaged from ≥2 files is shaded **grey**. Defaults to one layer so columns show immediately. Columns sized to content, no stretch. Independent of the panel-1 organizer **and of the replicate designation**; layers persist.
