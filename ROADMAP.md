...
- 🟡 Label the **MS1 progenitor isotopic distribution** and its **fragment isotopic compositions**, labeling the **ions by both type and number** (use `examples/peptidefragmentscoring.py`). ✅ Selecting a candidate in **Table 2** overlays its theoretical **b/y fragment ions** on the MS2 spectrum: ported `fragment_element_binomial_walk` / `fragment_descending_partial_products` into `viewer/fragments.py` (dividingthreshold 0.1, subisotopomericdepth 0.5); only the **precursor isotopes inside the MS2 isolation window** (from the mzML) seed the fragment-isotope walk, and each fragment is attributed to the MS1 isotope(s) it descended from. Matched ions plot **green**, absent ions **red** (both theme-adaptive), each labelled with the ion + `M+k` isotope at the top of its line in the theme fg colour. ⬜ the MS1 progenitor envelope twin-plot itself is still pending (1.10).
